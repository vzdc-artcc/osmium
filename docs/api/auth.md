...
- `GET /api/v1/auth/vatsim/login` accepts an optional `return_to` query param (an absolute URL) carrying the post-login redirect target across the OAuth round trip in an `osmium_oauth_return_to` cookie; the callback redirects there instead of `/api/v1/me` once login completes. `return_to`'s origin must be present in `CORS_ALLOWED_ORIGINS` or the login request is rejected with 400 — this reuses the CORS allowlist since it's the same trust boundary (frontends allowed to make credentialed calls are the frontends trusted as redirect targets). If `return_to` is omitted, or the cookie is missing/invalid by the time the callback runs, the flow falls back to `/api/v1/me`.
