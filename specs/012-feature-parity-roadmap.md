...
11. **GDPR self-service data export (right of access)** — not started. See the dedicated section above; has an open compliance question (trainer-note redaction for regular users) that should be confirmed with whoever owns compliance before implementation starts, not assumed from the initial ask.

**Remaining open work, all deliberately deferred or not yet started (not gaps I missed):** ATC booking proxy, self-hosted FAA preferred-routes data, specs 009-011, and the GDPR data-export feature (item 11). Every feature-parity item that was actually in scope, including the docs update, is now closed.
